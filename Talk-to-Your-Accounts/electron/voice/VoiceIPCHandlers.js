...
// Located in electron/ directory to be accessible in production builds

const { ipcMain, shell } = require('electron');
const path = require('path');

// Import voice services from local voice directory
...
async function initializeVoiceServices() {
  ...
  try {
    // Load from local voice directory (same directory as this file)
...
function setupVoiceHandlers(mainWindow) {
  ...
  ipcMain.handle('voice:toggle', async () => {
    try {
      if (!voiceService) return { success: false, error: 'Voice service not available' };
      const state = voiceService.toggleListening();
  ...
  function setupVoiceEventForwarding() {
    ...
    const events = [
      'initialized', 'listeningStarted', 'listeningStopped',
      'transcriptionComplete', 'partialTranscription', 'commandParsed',
      'commandReady', 'executeCommand', 'audioData', 'audioLevel',
      'error', 'modelChanged', 'settingsChanged', 'historyUpdated', 'enabledChanged'
    ];
...
async function executeVoiceCommand(command) {
  ...
  // Access global database (set in main.js)
  const db = global.db;
...
async function handleAddExpense(entities, rawText) {
  try {
    ...
    `).run('Voice Expense', amount, description, date);

    return {
      action: 'expense_added',
      expenseId: expenseId.lastInsertRowid,
...
async function handleNavigate(entities) {
  try {
    ...
    const pageMap = {
      ...
      'reports': 'Reports', 'parties': 'Parties',
