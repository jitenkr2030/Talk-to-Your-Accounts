...
class CommandParser extends EventEmitter {
  ...
  setupEntityExtractors() {
    this.entityExtractors.set('AMOUNT', [/(?:Rs\.?|INR|₹)?\s*([\d,]+(?:\.\d{1,2})?)/i]);
    this.entityExtractors.set('DATE', [/(?:on\s+)?(\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4})/i, /(?:on\s+)?(today|yesterday|tomorrow)/i]);
    this.entityExtractors.set('DESCRIPTION', [/(?:for|on|to)\s+([a-zA-Z\s]+?)(?:\s+(?:of|for|on)|$)/i]);
    this.entityExtractors.set('CATEGORY', [/(?:category\s+)?([a-zA-Z]+)/i]);
  }
  ...
  async parse(text) {
    ...
    return {
      command: { intent: 'UNKNOWN', entities: [], rawText: text },
  ...
  extractEntities(text) {
    ...
    if (amountMatch) {
      entities.push({ type: 'AMOUNT', value: parseFloat(amountMatch[1].replace(/,/g, '')), raw: amountMatch[0] });
    }
    const dateMatch = text.match(/(?:on\s+)?(\d{1,2}[-\/]\d{1,2}[-\/]\d{2,4})/i) || text.match(/(?:on\s+)?(today|yesterday|tomorrow)/i);
    if (dateMatch) {
      entities.push({ type: 'DATE', value: dateMatch[1], raw: dateMatch[0] });
    }
    const descMatch = text.match(/(?:for|on|to)\s+([a-zA-Z\s]+?)(?:\s+(?:of|for|on)|$)/i);
    if (descMatch) {
      entities.push({ type: 'DESCRIPTION', value: descMatch[1].trim(), raw: descMatch[0] });
    }
    return entities;
  }
...
class AudioRecorder extends EventEmitter {
  async initialize() { console.log('[AudioRecorder] Initialized'); }
  start() { return true; }
  stop() { return null; }
  getData() { return null; }
}
...
class Transcriber extends EventEmitter {
  async initialize() { console.log('[Transcriber] Initialized'); }
  async transcribe(audioData) { return { text: '', confidence: 0 }; }
  parseCommand(text) { return null; }
}
